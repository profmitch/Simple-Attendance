...
export class RosterRecordsManager {
    ...
    rosterFileNamesRE;
    rosterFileNames = [];
    rostersRecordsHeaders;
    rosterFilesRelpath;
    debugRosters; // used to debug roster files use
    constructor(appInfo) {
    ...
    getRosterRecordsFolder() {
        return this.rosterRecordsPath;
    }
    // gets a list of all the file names
    async getRosterFilesNames(verbose) {
        if (verbose && verbose == true)
            console.log("Searching for roster records in files" +
    ...
    async selectRosterFiles(rosterFileNames) {
        ...
        const rosterInquirerQuestions = [
            {
                ...
                default: () => {
                    ...
                    return rosterFileData.filter(f => f.datetime.toLocaleDateString() === latestDateStr);
                },
                pageSize: 10,
            }
        ];
    ...
    async getRosterFileData(fileNames) {
        let rosterFiles = [];
        for (const rosterFileName of fileNames)
            if (rosterFileName.search(/\.prn$/i) >= 0) { // USE THE SECTION ROSTER tool on SCCCD Dashboard 
                ...
                const prnFileContent = (await readFile(rosterPRNFileFullPath)).toString().replace(/\r/g, "").split("\n");
    ...
    #parseRosterPRNFile(prnFileContent) {
        let line = 0, parts;
        const prnFileInfo = {
            fileNameFullPath: "",
            datetime: new Date(),
    ...
    convertPRNFileInfo2RosterRecords(prnFileInfo) {
        const rosterRecords = [];
    ...
    #parseCSVRoster(rosterContent, csvFileName) {
        let headers = "";
    ...
    verifyRosterFile(fileContent) {
        ...
        const sortedHeaders = this.rostersRecordsHeaders.sort((a, b) => a > b ? 1 : a < b ? -1 : 0);
...
//# sourceMappingURL=rosterManagerClass.js.map