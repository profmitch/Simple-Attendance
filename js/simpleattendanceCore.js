...
function coreProcessing(appInfo, rosterCabinet, attendanceCabinet // keep the contents of all files separate as strings
) {
    ...
    */
    ...
    const sectionRosters = Object.entries(Object.groupBy(multiSort(rosterCabinet.records, [
        { key: "Section" },
        { key: "Name" }
    ]), (rec) => rec.Section)).map(([section, records]) => ({
    ...
    */
    ...
    let sessionCode = "", sessionLabSection, sessionRecord, sessionRecords = [], present = [], absent = [], unmatched = [], sessionType, sessionDate, csvRecords = [], sessionSRecords = [];
    rawSessionRecords = sortByKey(rawSessionRecords, "Attendance Code");
    const foundSessionCodes = [];
    let index = -1;
    ...
    for (let sessionRecords of sessionSRecords) {
        ...
        for (const sessionRecord of sessionRecords)
            if (rosterCabinet.records.find(rosRec => rosRec.StudentId == sessionRecord["Student ID"] //&&
